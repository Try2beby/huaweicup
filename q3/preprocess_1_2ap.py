...
df_2ap = pd.read_csv("../data/test_set_1_2ap.csv")
...
df_2ap["mcs_nss"] = df_2ap.apply(
    lambda row: f"{int(row['mcs'])}_{int(row['nss'])}", axis=1
)
...
print("unique values in ('mcs_nss', '_'):")
print(df_2ap[("mcs_nss", "_")].value_counts(), len(df_2ap[("mcs_nss", "_")].unique()))
...
# save
df_2ap.to_csv("./df_1_2ap_test_final.csv", index=False)
